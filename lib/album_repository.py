...
class AlbumRepository:
    ...
    def all(self):
        rows = self.connection.execute('SELECT * FROM albums ORDER BY id ASC')
        albums = []
        for row in rows:
    ...
    def update(self, album):
        self.connection.execute('UPDATE albums SET title = %s, release_year = %s, artist_id = %s WHERE id = %s', [album.title, album.release_year, album.artist_id, album.id])